...
```{r}
# load("~/Documents/R/SpatialfdaR/data/MeuseData.rda")
p <- MeuseData$pts
e <- MeuseData$edg
t <- MeuseData$tri
np <- dim(p)[[1]]
...
A hazard when the observations are all at the nodes is that the linear equation that must be solved in smoothing function `smooth.FEM.basis1` cannot be solved because the left side is not of full rank. A simple way around that is to impose some penalty on the roughness of the solution by setting roughness penaly `lambda` to a positive value.  Here is the command that produces a nice smooth surface by using `lambda = 1e-4`:
```{r}
...
The following commmands plot the fitted surface by using the powerful graphics capabilities of the 
`rgl` package.
...
In any case, the triangles needn't be as small they are in this analysis for the purposes of examining the variaton in the zinc deposits.  Reducing the mesh density is a good way to go. 